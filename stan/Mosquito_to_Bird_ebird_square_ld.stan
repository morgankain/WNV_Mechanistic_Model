...
parameters {
 ...
 // Random Effect Varaiances
    ...
    real<lower=0> sigmasq_beta_LD2_VS;
...
transformed parameters {
 ...
 // Square root for convenience
    ...
    real<lower=0> sigma_beta_LD2_VS;
 ...
 // 1 / Sqaure root
    ...
    sigma_beta_LD2_VS = 1.0 / sqrt(sigmasq_beta_LD2_VS);
 ...
 // Linear Predictor
     ...
     (beta_LD2 +		 		  // Log Dose quadrant for each virus lineage
     beta_LD2_VS_r[VS[i]]) *			  // random component of Log Dose (quadratic) for each Vector Species
     LD2[i] +					  // across Log Dose ^2
...
model {
 ...
 // Diffuse Priors on Random Effect variance
    ...
    sigmasq_beta_LD2_VS ~ gamma(1.0E-3, 1.0E-3);
  ...
  for (m in 1:N_VS) {
    ...
    beta_LD2_VS_r[m] ~ normal(0, sigma_beta_LD_VS);
