...
leave_one_out_only <- TRUE
uni_count <- as.character(unique(comm_comp_summary_p2_well_observed_m_dh[["CNTY_NM"]]))

if (leave_one_out_only == FALSE) {
  RMSE_est  <- data.frame(
  county   = numeric(0)
, observed = numeric(0)
, pred_f   = numeric(0)
...
for (i in seq_along(uni_count)) {
  ...
  comm_comp_spatio_temporal_f <- try(gamm(
    med_comp ~ 
    ## Spatial fixed
    s(X, Y) +
    s(NATRGN, bs = "mrf", xt = xt) + 
  ...
  comm_comp_spatio_temporal_r3 <- try(gamm(
    med_comp ~ 
    ## Spatial fixed
  # s(X, Y) +
    s(NATRGN, bs = "mrf", xt = xt) + 
  ...
  if ((class(comm_comp_spatio_temporal_f) != "try-error")[1] & (class(comm_comp_spatio_temporal_r3) != "try-error")[1]) {
